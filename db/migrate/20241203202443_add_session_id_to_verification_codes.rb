class AddSessionIdToVerificationCodes < ActiveRecord::Migration[8.0]
  def change
    ...
    # Populate the session_id column for existing verification codes
    VerificationCode.all.each do |vc|
      session = Session.find_by(user_id: vc.user_id)
      vc.update(session_id: session.id)
    end
